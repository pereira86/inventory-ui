...
def render_count_dashboard(session:dict,items:list[dict],go:Callable[[str],None],**kwargs)->None:
    counted=sum(i.get('status')!='not_counted' for i in items); total=len(items)
    ...
    if session.get('id'):
        if st.button('Revisar',use_container_width=True): go('review')
    else:
        st.button('Revisar',use_container_width=True,disabled=True,help='A revisão fica disponível depois do primeiro registro de contagem.')

    with st.container(key=f"nav_reference_dashboard_{session.get('location_id','draft')}"):
        c1,c2=st.columns(2)
        if c1.button('← Voltar',use_container_width=True,key=f"dash_back_{session.get('location_id','draft')}"):
            ...
            st.rerun()
        if c2.button('Início',use_container_width=True,key=f"dash_home_{session.get('location_id','draft')}"):
            go('home')
